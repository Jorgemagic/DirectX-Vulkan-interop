[Begin_ResourceLayout]
	...
	SamplerComparisonState DirectionalShadowMapSampler	: register(s10);
	Texture2DArray SpotShadowMap						: register(t11); [SpotShadowMap]
	SamplerComparisonState SpotShadowMapSampler			: register(s11);
	Texture2DArray PunctualShadowMap					: register(t12); [PunctualShadowMap]
	SamplerComparisonState PunctualShadowMapSampler		: register(s12);
...
[Begin_Pass:ShadowMap]
	...
	{
	#if ATEST && DIFF
		float alpha = BaseColorTexture.Sample(BaseColorSampler, input.TexCoord0).a;
...
[Begin_Pass:ZPrePass]
	...
	{
		float3      Position            : POSITION;
	#if NORMAL || MT_RG_TEXTURED || (ATEST && DIFF)
		float2      TexCoord0           : TEXCOORD0;
	#endif
	...
	void DirectionalLight(const ShadingParams shading, const MaterialInputs material, const PixelParams pixel, const LightProperties lightProperties, inout float3 color, in float depthVS)
	{
		...
		if (NoL > 0)
		{
			...
			if(lightProperties.IsCastingShadow())
			{
				...
				shadowTerm = ShadowCascade(DirectionalShadowMap, DirectionalShadowMapSampler, lightProperties, shadowPosition.xyz, cascade);
			}
		#endif		
